...
% S:[1,0], W:[0,-1].
time=0;
j=0; %this is the spiral counter to keep track of where we are in the (spiralling) algorithm that we apply when there's no food ahead.
...
for t=1:tmax
    ...
    if ahead(G,A,n)==1 %if food ahead then move forward and eat it
        ...
        j=0; %reset the spiral counter
    else % begin spiralling (first checking around)
        j=j+1;
         if ismember(j,[1 2 3 4 6 8 11 14 18 22 27 32 38 44 51 58 66 74 ...
                 83 92 102 112 125 138 152 166 181 196 213 ])==1 %then turn left
                A(2,:)=([0,-1;1,0]*A(2,:)')';  
            else %move forwards
                A(1,:)=mod(A(1,:)+A(2,:),n);
         end
