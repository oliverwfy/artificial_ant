...
for s=1:simulation_times
    ...
    for t=1:tmax
        time=time+1;
        % Check if there's food ahead of the ant.
        ...
        % If there is food in front of the ant then move the ant forward one place:
        if G(fP(1),fP(2))==1 
            A(1,:)=mod(A(1,:)+A(2,:),n); %this moves the ant forward one place
       
        % If there's no food infront of the ant then the ant performs a random
        % action.
        else
...
histogram(scores)
xlabel('Food pieces found', FontSize=20)
ylabel('Number of runs of simulations', FontSize=20)
saveas(gcf,'histBasicSensing500.png')
